...
The status panel distinguishes these modes. `RunPod local CUDA detected` means `boltr-web` appears to be running directly on a RunPod host and will use that pod's attached GPU without SSH. `RunPod SSH connected` means the web server reached `BOLTR_RUNPOD_HOST` over SSH. `Local CUDA detected (no RunPod SSH)` means `BOLTR_RUNPOD_HOST` is unset and the web server found an ordinary local GPU. If a pod image does not expose standard `RUNPOD_*` environment variables, set `BOLTR_RUNPOD_LOCAL=1` before starting `boltr-web` to force the RunPod-local label.
