...
*Last updated: 2026-03-25 — `with_dev_venv.sh` / `cargo-tch` set `LD_LIBRARY_PATH` for PyTorch LibTorch runtimes; §3 hyperparameter row aligned with `Boltz2Hparams` + export script; collate→`predict_step_trunk` called out in §4.5 / §7 / progress log. Still open: full `predict_step`, real `TemplateV2Module`, §5.1 full-checkpoint VarStore audit, full `load_input`, featurizer collate allclose golden.*
