...
| **3a** | Dummy templates + merged `FeatureBatch` | **Done** | [`dummy_templates.rs`](boltr-io/src/featurizer/dummy_templates.rs); [`trunk_smoke_feature_batch_from_inference_input`](boltr-io/src/inference_dataset.rs) merges token + MSA + **atom** ([`atom_features_from_inference_input`](boltr-io/src/inference_dataset.rs)) + dummy templates (no `s_inputs`). Test: [`trunk_smoke_feature_batch_covers_collate_manifest_keys`](boltr-io/tests/load_input_dataset.rs). |
...
| **4** | `process_atom_features` | **[~]** | **Rust:** [`process_atom_features.rs`](boltr-io/src/featurizer/process_atom_features.rs); **inference:** [`atom_features_from_inference_input`](boltr-io/src/inference_dataset.rs) + merge in [`trunk_smoke_feature_batch_from_inference_input`](boltr-io/src/inference_dataset.rs). **Golden:** schema + [`atom_features_ala_rust_matches_python_golden_allclose`](boltr-io/src/featurizer/atom_features_golden.rs) (allclose for keys not depending on RDKit NPZ/conformer; skips `ref_chirality`, `coords`, `ref_pos`, `disto_coords_ensemble`, `disto_target`). **TBD:** full-key allclose vs Python on **same** structure NPZ + mols; ligands via custom [`AtomRefDataProvider`](boltr-io/src/featurizer/process_atom_features.rs). |
...
| 2026-03-27 | **Code review + build hygiene** | Restored corrupted [`featurizer/mod.rs`](boltr-io/src/featurizer/mod.rs) (re-exports for `lib.rs`). Fixed `chirality_type_id` **i32→i64** in [`process_atom_features.rs`](boltr-io/src/featurizer/process_atom_features.rs). Added missing [`minimal_protein.yaml`](boltr-io/tests/fixtures/minimal_protein.yaml) for [`yaml_parse` test](boltr-io/tests/yaml_parse.rs). Superseded for atoms by **§2b phase 4** row below. |
| 2026-03-27 | **§2b phase 4 — inference merge + atom golden** | [`atom_features_from_inference_input`](boltr-io/src/inference_dataset.rs); [`trunk_smoke_feature_batch_from_inference_input`](boltr-io/src/inference_dataset.rs) merges atom `FeatureBatch`; [`featurizer/mod.rs`](boltr-io/src/featurizer/mod.rs) / [`lib.rs`](boltr-io/src/lib.rs) export `process_atom_features`, `AtomFeatureTensors`, `AtomRefDataProvider`, `StandardAminoAcidRefData`, `AtomFeatureConfig`, `inference_ensemble_features`. **Tests:** [`atom_features_ala_rust_matches_python_golden_allclose`](boltr-io/src/featurizer/atom_features_golden.rs); [`manifest.json`](boltr-io/tests/fixtures/collate_golden/manifest.json) `atom_features_ala_golden_keys`; [`collate_golden_fixture.rs`](boltr-io/tests/collate_golden_fixture.rs) `atom_features_ala_golden_has_manifest_keys`. |
...
| [~] | `process_atom_features` | same | **Rust + inference:** [process_atom_features.rs](boltr-io/src/featurizer/process_atom_features.rs); [`atom_features_from_inference_input`](boltr-io/src/inference_dataset.rs); merged in [`trunk_smoke_feature_batch_from_inference_input`](boltr-io/src/inference_dataset.rs). **Golden:** schema + partial allclose ([atom_features_golden.rs](boltr-io/src/featurizer/atom_features_golden.rs), skip list for RDKit/geometry keys). **TBD:** full-key allclose on aligned NPZ + mols; ligands/CCD providers. |
...
**Acceptance (§4.4 product bar):** For one **frozen** Python export (`dict[str, Tensor]` after `collate`), Rust produces **allclose** tensors (document rtol/atol per key). **Done for:** token (ALA), MSA (`load_input_smoke`), atoms **partial** (inference merge + subset allclose vs `atom_features_ala_golden.safetensors`; geometry/RDKit keys skipped until NPZ alignment). **Remaining:** full post-collate dict for variable-`N` batch, full atom allclose on same inputs as Python, real templates.
...
1. **Featurizer team (current bottleneck):** Follow **§2b** — **next:** **real** `process_template_features` (§2b **3b**), then **full collate dict** golden (§2b phase 5); extend atom allclose to full keys on **same** NPZ + mols as Python. §4.1–4.2 still block full YAML/CCD parity.
...
*Last updated: 2026-03-27 — **§2b:** [`atom_features_from_inference_input`](boltr-io/src/inference_dataset.rs) + atom merge in [`trunk_smoke_feature_batch_from_inference_input`](boltr-io/src/inference_dataset.rs); partial atom **allclose** ([`atom_features_golden.rs`](boltr-io/src/featurizer/atom_features_golden.rs)); [`manifest.json`](boltr-io/tests/fixtures/collate_golden/manifest.json) `atom_features_ala_golden_keys`. **Still open:** full atom allclose on aligned fixtures, real templates (§2b **3b**), full trunk post-collate dict vs Python, full schema/CCD, diffusion/confidence/affinity, **`predict_step`**, writers, CLI wiring.*
